...
public strictfp class LineCircleCollider implements Collider {
	...
	public int collide(Contact[] contacts, Body bodyA, Body bodyB) {
		...
		Vector[] vertsA = line.getVertices(bodyA.getPosition(), bodyA
				.getRotation());
		...
		if (uA < 0) { // the intersection is somewhere before startA
			...
			position = new Vector(startA.x + uA * (endA.x - startA.x), startA.y
					+ uA * (endA.y - startA.y));
		}
		...
		if (distSquared < radiusSquared) {
			...
			float separation = (float) Math.sqrt(distSquared)
					- circle.getRadius();
