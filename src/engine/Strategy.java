...
public class Strategy {
	...
	public void collideBodies(CollisionContext context, BodyList bodies,
		...
		for (int i = 0; i < bodies.size(); i++) {
			...
			space.addAABox(body.getShape().getBounds(), body.getPosition()
					.getX(), body.getPosition().getY());
			space.addBody(body);
		}
	 ...
	 */
	private boolean splitSpace(Space space, int level, int target,
	...
	public class Space extends BodyList {
		...
		public Space[] getQuadSpaces() {
			...
			for (int i = 0; i < size(); i++) {
				Body body = get(i);
				for (int j = 0; j < 4; j++) {
					if (spaces[j].touches(body.getShape().getBounds(), body
							.getPosition().getX(), body.getPosition().getY())) {
						spaces[j].add(body);
					}
				}
			}
