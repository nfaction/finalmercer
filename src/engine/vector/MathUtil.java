...
/**
 * Simple utility wrapping up a bunch of math operations so that the rest of the
 * code doesn't have to look so cluttered.
 * 
 * @author Jeff Ahern
...
public final strictfp class MathUtil {
	...
	public static Vector mul(Vector2D A, Vector v) {
		return new Vector(A.col1.x * v.getX() + A.col2.x * v.getY(), A.col1.y
				* v.getX() + A.col2.y * v.getY());
	}
	 ...
	 * 
	 * @param A
	 *            The 2dVecotor to make absolute
	 ...
	 *            The scalar float
	 * @param a
	 *            The vector to fidn the cross of
