...
public strictfp class BoxCircleCollider implements Collider {
	...
	public int collide(Contact[] contacts, Body boxBody, Body circleBody) {
		...
		boolean touches = boxBody.getShape().getBounds().touches(x1, y1,
				circleBody.getShape().getBounds(), x2, y2);
		if (!touches) {
			return 0;
		}
		...
		Vector[] pts = box.getPoints(boxBody.getPosition(), boxBody
				.getRotation());
		...
		if (closest > -1) {
			...
			Vector normal = MathUtil
					.sub(circleBody.getPosition(), contactPoint);
			normal.normalise();
