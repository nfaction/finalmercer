...
/**
 * Collides two lines with one another.
 * 
 * @author Jeff Ahern
...
public class LineLineCollider implements Collider {
	...
	/**
	 * Gets the closest point to a given point on the indefinately extended
	 * line.
	 * 
	 * @param startA
	 ...
	 * @param point
	 *            The point to get a closes point on the line for
	 * @return the closest point on the line or null if the lines are parallel
	 */
	public static Vector getClosestPoint(Vector startA, Vector endA,
			Vector point) {
		Vector startB = point;
		...
		return new Vector(startA.x + uA * (endA.x - startA.x), startA.y + uA
				* (endA.y - startA.y));
	}
